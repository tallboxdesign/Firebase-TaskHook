...
import { ListChecks, Settings, CalendarDays, CheckSquare, Archive, Star, Filter, Sparkles, PanelLeft, AlertTriangle } from "lucide-react";
...
import type { Timeframe, SettingsValues } from "@/lib/types";
...
interface AppHeaderProps {
  ...
  settings?: SettingsValues; // Added settings prop
}
    ...
    <header className="bg-white border-b border-slate-200 px-4 py-3 sticky top-0 z-50 shadow-sm">
      <div className="max-w-7xl mx-auto flex items-center justify-between">
        ...
        <div className="flex items-center space-x-2">
          {settings?.disableIncomingWebhookAuth && (
            <TooltipProvider>
              <Tooltip>
                <TooltipTrigger asChild>
                  <div className="p-1.5 text-yellow-500 flex items-center">
                    <AlertTriangle size={18} className="mr-1" />
                    <span className="text-xs font-medium">Auth Disabled</span>
                  </div>
                </TooltipTrigger>
                <TooltipContent>
                  <p className="text-sm">Incoming webhook authentication is currently disabled. This is a security risk.</p>
                </TooltipContent>
              </Tooltip>
            </TooltipProvider>
          )}
